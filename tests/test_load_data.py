...
def test_embedding_load_data_source_fixed_size_chunks(
    chunk_size: int, background_is_constrained_source: bool, use_additional_files: bool
) -> None:
    ...
    full_size_source = track_skim.FileSource(
        filename=_track_skim_base_path
        / "reference"
        / f"AnalysisResults__{_constrained_collision_system}__jet_R020.root",
    ...
    full_file_size = len(next(full_size_source.gen_data())) * (
        _n_repeat_background if background_is_constrained_source else _n_repeat_pythia
    )
