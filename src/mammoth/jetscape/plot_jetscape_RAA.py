...
def _ML_jet_binning(system: str, jet_R: float) -> npt.NDArray[np.float64]:
    ...
    new_bins = np.concatenate(
        [
            np.arange(min_pt_values[system][jet_R], 70, 10),
            np.arange(70, 100, 15),
    ...
    with sns.color_palette("Set2"):
        for jet_R in jet_R_values:
            for jet_type in jet_types:
                ...
                text = rf"{jet_type.capitalize()} jets, $R$ = {jet_R}"
    ...
    for jet_type in jet_types:
        for system in ["PbPb_00_10", "PbPb_30_50"]:
            for restricted_range in [False, True]:
                text = rf"{labels[system]}, {jet_type.capitalize()} jets"
                # Just for some user feedback
                print(text)  # noqa: T201
                ...
                for jet_R_label, jet_R_values_to_iterate in [
                    ("", jet_R_values),
                    ("_alice_comparison", [0.2, 0.4]),
                    ("_requested", [0.2, 0.4, 0.6]),
                ]:
                        ...
                        panels=[
                            pb.Panel(
                                axes=[
                                    ...
                                    pb.AxisConfig(
                                        "x", label=r"$p_{\text{T,jet}}\:(\text{GeV}/c)$", font_size=22, **x_axis_kwargs
                                    ),
                                ],
                                text=pb.TextConfig(x=0.97, y=0.03, text=text, font_size=22),
                                legend=pb.LegendConfig(location="upper right", font_size=22),
