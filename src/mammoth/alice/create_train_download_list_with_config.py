...
relatively easy to hack, so no point in messing with it (as of June 2023).
...
# NOTE: "stages" was the original name - it's generalized a bit from that
#       (eg. supporting run_by_run), but the name is kept for backwards compatibility.
...
def add_files_from_xml_file(
    ...
    """Extract file pairs based on the given XML file.

    Args:
        alien_xml_file: Path to the XML file on alien.
        local_xml_file: Path to the XML file on the local filesystem.
        local_train_dir: Path to the local train directory.
        child_label: Label for the child train.
        additional_label: Additional label to add to the file name. Default: ""
    Returns:
        File list from alien to local filename.
    """
    output = {}
    ...
    for train_number in trains:
        ...
        for _child_name, child_info in alien_output_info.items():
            ...
            if stage_to_download == "merged":
                ...
                # NOTE: For the specific dataset, imagine LHC23a3, which splits the CENT, FAST, etc
...
def entry_point() -> None:
    ...
    create_train_download_list(trains=trains)
