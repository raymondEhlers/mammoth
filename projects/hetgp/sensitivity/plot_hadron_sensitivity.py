...
for index in range(len(filenameindex)):
    filename_HF = "HFGP/Hadron aggregate/Global_TotalIndex_" + filenameindex[index] + ".csv"
    HFdatabyaggbin = pd.read_csv(filename_HF)
    HF = np.array(HFdatabyaggbin['STagg'])
    HFerr = np.array(HFdatabyaggbin['ST_confagg'])
    filename_HetGP = "HetGP/Hadron aggregate/Global_TotalIndex_" + filenameindex[index] + ".csv"
    ...
    plotname = "Figures/Hadron_Global_TotalIndex_aggbin_" + filenameindex[index] + ".pdf"
    plot(HF, HetGP, HFerr, HetGPerr, plotname)

# %%
# local 5-95 total index
for index in range(len(filenameindex)):
    filename_HF = "HFGP/Hadron aggregate/Local_5_95_TotalIndex_" + filenameindex[index] + ".csv"
    HFdatabyaggbin = pd.read_csv(filename_HF)
    HF = np.array(HFdatabyaggbin['STagg'])
    HFerr = np.array(HFdatabyaggbin['ST_confagg'])
    filename_HetGP = "HetGP/Hadron aggregate/Local_5_95_TotalIndex_" + filenameindex[index] + ".csv"
    HetGPdatabyaggbin = pd.read_csv(filename_HetGP)
    HetGP = np.array(HetGPdatabyaggbin['STagg'])
    HetGPerr = np.array(HetGPdatabyaggbin['ST_confagg'])
    plotname = "Figures/Hadron_Local_5_95_TotalIndex_aggbin_" + filenameindex[index] + ".pdf"
    plot(HF, HetGP, HFerr, HetGPerr, plotname)

# %%
# local 1-99 total index
for index in range(len(filenameindex)):
    filename_HF = "HFGP/Hadron aggregate/Local_1_99_TotalIndex_" + filenameindex[index] + ".csv"
    HFdatabyaggbin = pd.read_csv(filename_HF)
    HF = np.array(HFdatabyaggbin['STagg'])
    HFerr = np.array(HFdatabyaggbin['ST_confagg'])
    filename_HetGP = "HetGP/Hadron aggregate/Local_1_99_TotalIndex_" + filenameindex[index] + ".csv"
    HetGPdatabyaggbin = pd.read_csv(filename_HetGP)
    HetGP = np.array(HetGPdatabyaggbin['STagg'])
    HetGPerr = np.array(HetGPdatabyaggbin['ST_confagg'])
    plotname = "Figures/Hadron_Local_1_99_TotalIndex_aggbin_" + filenameindex[index] + ".pdf"
    plot(HF, HetGP, HFerr, HetGPerr, plotname)
